...
export class ProfileComponent implements OnInit, OnDestroy {
  ...
  loadUserData(): void {
    const workspaceId = this.workspaceState.getWorkspaceSync()?.workspaceId;
    ...
    forkJoin({
      contacts: this.contactsService.getMyContacts(workspaceId).pipe(
        catchError(err => {
      ...
      next: ({ contacts, campaigns, templates }) => {
        // Store all data (filtering will be done by backend APIs)
  ...
  private processChartData(campaigns: any[], contacts: any[], templates: any[]): void {
    ...
    // Update bar chart data (Contacts Added Over Time)
    this.barChartData = {
      labels: months,
      datasets: [
        {
